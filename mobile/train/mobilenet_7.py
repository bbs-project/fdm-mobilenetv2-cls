...
parser.add_argument('--model_num', default='7')             # ← 이어학습 시 현재 번호 (예: 7)
...
parser.add_argument('--d21_multi_ratio', type=float, default=0.35)  # 데이터상 0%여도 안전하게 유지
...
parser.add_argument('--unfreeze_from', type=int, default=-40)
...
def build_indices_for_training(y, max_d21, oversample_factors, d21_multi_ratio=0.35):
    y = np.asarray(y)
    # d21-only / d21-multi 분리 (multi가 0%여도 안전)
...
def safe_resume_from_prev(model, args, model_dir_current):
    """
    이전 번호 폴더(예: mobilenet_6)에서 best_model→saved_weights 순으로 조용히 로드.
    불일치 변수는 허용(expect_partial), threshold/temperature 시드도 복사.
    리턴: (resumed(bool), used_ckpt_path(str or None))
    """
    ...
    try:
        model.build((None, 224, 224, 3))  # 변수 생성 보장
    except Exception:
        pass
