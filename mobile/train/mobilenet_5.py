...
# ===================== Args =====================
parser = argparse.ArgumentParser(description='RGB 다중질병 (MobileNetV2, ASL train + weighted-BCE val)')
...
parser.add_argument('--model_num', default='5')  # ← update
...
parser.add_argument('--top_k', type=int, default=3)  # ← default 3 (최대 3개 라벨 허용)
parser.add_argument('--max_d21', type=int, default=2400)
parser.add_argument('--d21_multi_ratio', type=float, default=0.35)  # 21-multi 상한 비율
parser.add_argument('--d21_min_thr', type=float, default=0.60)      # ← d21 임계값 하한
...
parser.add_argument('--gamma_neg', type=float, default=4.0)   # default 3.0 -> 4.0
parser.add_argument('--neg_clip', type=float, default=0.08)   # default 0.04 -> 0.08
...
def build_indices_for_training(y, max_d21, oversample_factors, d21_multi_ratio=0.35):
    y = np.asarray(y)
    # d21-only / d21-multi 분리 후 multi 비율 제한
...
class ClasswiseThresholdTuner(Callback):
    def __init__(self, x_val, y_val, every=3, grid=np.arange(0.30, 0.96, 0.01), iters=1,
                 save_dir=None, top_k=None, d21_min_thr=None):
        super().__init__()
        self.x_val = x_val
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # d21 하한 적용
        if self.d21_min_thr is not None:
            thr[7] = max(thr[7], float(self.d21_min_thr))
...
def search_best_threshold(model, x_val, y_val, temp_scale=None, d21_min_thr=None):
    ...
    # d21 하한 적용
    if d21_min_thr is not None:
        thr[7] = max(thr[7], float(d21_min_thr))
...
def main(args):
    ...
    callbacks1 = [
        ...
        ClasswiseThresholdTuner(x_val=normalize_01(x_valid), y_val=y_valid, every=3,
                                grid=np.arange(0.30,0.96,0.01), iters=1,
                                save_dir=model_dir, top_k=TOP_K, d21_min_thr=args.d21_min_thr),
    ]
    ...
    # ----- 최종 threshold 저장 (온도보정 적용 + d21 하한) -----
    best_thr = search_best_threshold(model, x_val_n, y_valid, temp_scale=T, d21_min_thr=args.d21_min_thr)
