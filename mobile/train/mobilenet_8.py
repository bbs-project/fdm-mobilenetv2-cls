# mobilenet_train_resume_to_8.py
import os
import argparse
import numpy as np
import pandas as pd
...
# ===================== Args =====================
parser = argparse.ArgumentParser(description='RGB 다중질병 (MobileNetV2, ASL train + weighted-BCE val, 안전 이어학습 포함)')
...
parser.add_argument('--model_num',      default='8')             # ← 7에서 자동 재개하여 8로 저장
parser.add_argument('--model_name',     default='mobilenet')
parser.add_argument('--gpu_num',        default="0,1,2,3")

parser.add_argument('--delta',          type=float, default=1e-5)
parser.add_argument('--patience',       type=int,   default=20)   # ↑ 조금 길게 탐색
...
parser.add_argument('--warmup_epochs',  type=int,   default=6)     # ← 이어학습 안정용 기본값 조정
parser.add_argument('--unfreeze_from',  type=int,   default=-40)   # 마지막 40개 레이어 학습
parser.add_argument('--stage2_lr',      type=float, default=1.5e-5)# ← 2단계 더 보수적 학습률
...
EPS = 1e-7
TOP_K = args.top_k
THRESHOLD_VAR = None  # strategy.scope 안에서 초기화
...
def stable_asl_train_loss(pos_w, neg_w, gamma_pos=0.0, gamma_neg=4.0, neg_clip=0.08, label_smoothing=0.02):
    ...
    def loss_fn(y_true, logits):
        ...
        ce = tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true, logits=logits)  # [B,C]
        ce = _sanitize(ce, 0.0)
...
def weighted_bce_val_loss(pos_w, neg_w, label_smoothing=0.0):
    ...
    def loss_fn(y_true, logits):
        ...
        ce = tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true, logits=logits)  # [B, C]
        w  = y_true * pos_w + (1.0 - y_true) * neg_w                               # [B, C]

        num = tf.reduce_sum(w * ce, axis=0)     # per-class weighted sum
...
def set_backbone_trainable(model: tf.keras.Model, from_idx: int):
    ...
    if layers_ is None:
        try:
            layers_ = model.get_layer('MobilenetV2').layers
        except Exception:
            raise RuntimeError("Backbone layers not found.")
...
class ASLModel(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self.compiled_loss(y, y_pred)  # compile(loss=ASL)
    ...
    def test_step(self, data):
        ...
        val_loss = self.loss_val_fn(y, y_pred)    # 안정적인 BCE(클래스별 가중 평균)
        self.compiled_metrics.update_state(y, y_pred)
        results = {m.name: m.result() for m in self.metrics}
        results["loss"] = val_loss                # Keras가 val_ 접두사 붙여 로그 기록
        return results
...
def build_indices_for_training(y, max_d21, oversample_factors, d21_multi_ratio=0.35):
    ...
    d21_multi = d21_multi[: int(len(d21_only) * d21_multi_ratio)]
    d21_idx = np.concatenate([d21_only, d21_multi])
...
class ClasswiseThresholdTuner(Callback):
    def __init__(self, x_val, y_val, every=2, grid=np.arange(0.30, 0.96, 0.01), iters=2, save_dir=None, top_k=None):
        super().__init__()
        self.x_val = x_val
...
# ===================== 안전 이어학습 로더 =====================
def safe_resume_from_prev(model, args, model_dir_current):
    cur = int(args.model_num)
    name = getattr(args, "model_name", "mobilenet")
    root = args.checkpoint_path

    candidates = []
    for n in range(cur-1, 0, -1):
        prev_dir = os.path.join(root, f"{name}_{n}")
        if os.path.isdir(prev_dir):
            ck1 = os.path.join(prev_dir, "best_model")
            ck2 = os.path.join(prev_dir, "saved_weights")
            for ck in (ck1, ck2):
                if os.path.exists(ck + ".index") or os.path.exists(ck + ".h5"):
                    candidates.append((prev_dir, ck))
            if candidates:
                break

    if not candidates:
        print("[INFO] No previous checkpoint to resume.")
        return False, None

    prev_dir, ckpt = candidates[0]
    ...
    print(f"[INFO] AUTO-RESUME on {name}_{cur}  ←  {ckpt}")

    os.makedirs(model_dir_current, exist_ok=True)
    for fname in ["best_threshold.npy", "temp_scale.npy"]:
        src = os.path.join(prev_dir, fname)
        dst = os.path.join(model_dir_current, fname)
        if os.path.exists(src) and not os.path.exists(dst):
            try:
                np.save(dst, np.load(src))
                print(f"[INFO] Seeded {fname} from {prev_dir}")
            except Exception as e:
                print(f"[WARN] Could not seed {fname}: {e}")
...
def main(args):
    ...
    # 13/21 FP 억제: pos 완화, neg 강화
    pos_w[[5, 7]] = np.minimum(pos_w[[5, 7]], 1.2)
    neg_boost = np.ones_like(neg_w, np.float32); neg_boost[[3,5,7]] = [1.15, 1.35, 1.50]
    ...
    with strategy.scope():
        ...
        if args.use_adamw:
            ...
            try:
                ...
                opt = tf.keras.optimizers.Adam(learning_rate=lr_schedule, beta_1=0.9, beta_2=0.999, epsilon=1e-7)
        ...
        # ===== 안전한 이어학습 시도 =====
        resumed, used_ckpt = safe_resume_from_prev(model, args, model_dir)

        # 이어학습이면 학습률/워밍업/스테이지2 LR 보수적으로 낮춤
        if resumed:
            try:
                if hasattr(model.optimizer, "learning_rate"):
                    lr_now = float(tf.keras.backend.get_value(model.optimizer.learning_rate))
                    tf.keras.backend.set_value(model.optimizer.learning_rate, max(lr_now * 0.5, 2e-6))
            except Exception:
                pass
            warmup_epochs = min(max(1, args.warmup_epochs // 2), 3)
            stage2_lr = max(args.stage2_lr * 0.5, 2e-6)
            print(f"[INFO] Resume detected → LR={float(tf.keras.backend.get_value(model.optimizer.learning_rate)):.2e}, "
                  f"warmup_epochs={warmup_epochs}, stage2_lr={stage2_lr:.2e}")
        else:
    ...
    callbacks1 = [
        ...
        ClasswiseThresholdTuner(x_val=normalize_01(x_valid), y_val=y_valid, every=2,
                                grid=np.arange(0.30,0.96,0.01), iters=2, save_dir=model_dir, top_k=TOP_K),
    ]
    ...
    with strategy.scope():
        ...
        if args.use_adamw:
            try:
                opt2 = tf.keras.optimizers.AdamW(learning_rate=stage2_lr, weight_decay=args.weight_decay)
            except Exception:
                opt2 = tf.keras.optimizers.experimental.AdamW(learning_rate=stage2_lr, weight_decay=args.weight_decay)
        else:
            try:
                opt2 = tf.keras.optimizers.legacy.Adam(learning_rate=stage2_lr, beta_1=0.9, beta_2=0.999, epsilon=1e-7)
            except Exception:
                opt2 = tf.keras.optimizers.Adam(learning_rate=stage2_lr, beta_1=0.9, beta_2=0.999, epsilon=1e-7)

        model.compile(
            optimizer=opt2,
            loss=loss_train_fn,   # train은 계속 ASL
            metrics=[accuracy_logits, precision, recall, micro_f1],
        )
